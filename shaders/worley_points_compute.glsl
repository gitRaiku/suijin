...
void main() {
  if (dimensions == 2) {
    ...
    T(a.a, gi.x, gi.y, gi.z, udx, udy, 0) = float(gi.x * scale + gnoise(gi.xy * scale + vec2(4.1, 9.1)) * scale);
    T(a.a, gi.x, gi.y, gi.z, udx, udy, 1) = float(gi.y * scale + gnoise(gi.yz * scale + vec2(4.1, 9.1) + vec2(100000)) * scale);
    T(a.a, gi.x, gi.y, gi.z, udx, udy, 2) = float(gi.z * scale + gnoise(gi.xz * scale + vec2(4.1, 9.1) - vec2(100000)) * scale);
  }
}
